...
QString Time::toString() const {
    ...
    if (hours == 0) {
        const int minutes = (m_seconds % m_secs_per_hour) / m_secs_per_minute;
        const int seconds = m_seconds % m_secs_per_minute;
        return QStringLiteral("%1:%2")
                .arg(QString::number(minutes), 2, QLatin1Char('0'))
                .arg(QString::number(seconds), 2, QLatin1Char('0'));
    }

    return toString(QStringLiteral("%1:%2:%3"));
}

QString Time::toString(const QString &format) const {
    const int hours = m_seconds / m_secs_per_hour;
    const int i = m_seconds % m_secs_per_hour;
